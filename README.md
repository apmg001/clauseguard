...
> evaluation harness scores **100% precision/recall on a labeled synthetic set**;
> the deterministic tiers' real-world ceiling is documented honestly (see
> [Real-document testing](#real-document-testing)). OCR for scanned images is the
> remaining ingestion frontier and slots in behind the existing port.
...
*(Synthetic demo data. The pipeline has also been run against a real GST invoice —
see [Real-document testing](#real-document-testing).)*
...
measures real coverage, not a flattering subset. These scores are on controlled
**synthetic** cases; they validate the rules, not real-world extraction. For that,
...
The synthetic scores above validate the rules but say nothing about real-world
extraction, so the pipeline was also run against a **genuine third-party GST
invoice** (not generated by this project) — an Indian GST tax invoice with five
line items, per-line CGST/SGST, and a ₹12,185.86 total.

**Result: both deterministic tiers correctly *failed*** rather than emitting a
wrong `Invoice`, and reported exactly why:

```
LayoutAwareInvoiceExtractor: No line-item table recovered ...
RuleBasedInvoiceExtractor:   No line-item table header found in invoice text
```

Root causes (all real-world, all diagnosable):

1. **Collapsed table geometry** — `pdfplumber` recovered the item table as a
   single cell; the PDF has no ruling lines for the geometry tier to latch onto.
2. **Doubled-glyph header** — the header extracted as `IItteemm GGSSTT ...`
   (overlapping bold glyphs duplicated in the text layer), so no column matches.
3. **Different schema** — `Item / GST Rate / Qty / Rate / Amount / CGST / SGST /
   Total`, with no SKU (item name *is* the description) and tax columns the model
   doesn't represent.
4. **Unlabelled headers** — `Invoice No A00001` / `Billed By` (no colons).

This is the **designed ceiling** of deterministic parsing, and exactly why the
architecture has an **LLM tier** — reading a document whose layout defeats rules
is the "genuine ambiguity" this project reserves for a model. The system
degraded gracefully and produced no wrong output; it failed loud. Full analysis:
[`docs/REAL_DOCUMENT_TESTING.md`](docs/REAL_DOCUMENT_TESTING.md), reproducible
with `scripts/inspect_document.py`.
...
- **Fail loud, never silently wrong.** Extraction raises on any unparseable or
  ...
  wrong number is far more dangerous than a loud failure. (See Real-document
  testing — a real GST invoice failed loud rather than producing garbage.)
...
scripts/inspect_document.py   per-stage diagnostic for a real document
docs/REAL_DOCUMENT_TESTING.md  real-invoice test + root-cause analysis
```
