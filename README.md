...
The scores above are synthetic. The pipeline was also run against a **real GST
invoice** (a third-party digital PDF, not generated by this project): both
deterministic tiers correctly *failed* — collapsed table geometry, doubled-glyph
headers, and a GST schema the deterministic parsers don't cover. This is the
designed ceiling of rule-based extraction and exactly why the LLM tier exists.
The full analysis is in [`docs/REAL_DOCUMENT_TESTING.md`](docs/REAL_DOCUMENT_TESTING.md),
reproducible with `scripts/inspect_document.py`.
---
